...
(deftest test-sync-writes
  ...
  (let [req  (mock/http-servlet-request {} "")
        ...
        ;; Flush
        _    (.flush (.getWriter resp))]
    (is (= "Foobar" (slurp (.toByteArray (:output-stream resp))))))
...
(deftest test-async-read
  ...
  ;; Encoding not set
  (let [req (mock/http-servlet-request {} "Foobar")
        res (.get (b/read-body-as-string-async! req))]
    (is (= "Foobar" res)))
  
  ;; Works with encoding set.
  (let [req (-> {:character-encoding "utf-16"}
                (mock/http-servlet-request "やばい" {:encoding "utf-16"}))
        res (.get (b/read-body-as-string-async! req))]
    (is (= "やばい" res)))

  ;; Should fail when async not supported.
  (let [req (mock/http-servlet-request {:async-supported? false} "Foobar")]
...
(deftest test-async-write
  ...
  (let [req  (mock/http-servlet-request {} "")
        ...
        ret  (b/write-body body ctx)
        res  (slurp (.toByteArray (:output-stream resp)) :encoding "ISO-8859-1")]
    (is (b/async-body? body ctx))
    (is (instance? CompletionStage ret))
    (is (= "Foobar" res)))
  ...
  (let [req  (mock/http-servlet-request {} "")
        resp (mock/http-servlet-response {:character-encoding "utf-16"} req)
        ctx  {:serval.service/request  req
              :serval.service/response resp}
        _    (.startAsync req)
        body (b/async-body "Foobar")
        ret  (b/write-body body ctx)
        res  (slurp (.toByteArray (:output-stream resp)) :encoding "utf-16")]
    (is (b/async-body? body ctx))
    (is (instance? CompletionStage ret))
    (is (= "Foobar" res)))

  ;; String with async disabled
  (let [req  (mock/http-servlet-request {:async-supported? false} "")
        resp (mock/http-servlet-response {} req)
        ctx  {:serval.service/request  req
              :serval.service/response resp}
        body (b/async-body "Foobar")
